...
session_start();
    ...
    // $sql1="SELECT * FROM `register` where `role` IN ('3','4') && `status`='0' && `id`!= $id";
    $sql1="SELECT r.*, roles.role_name FROM `register` r JOIN `roles` ON r.`role` = roles.`role_id`WHERE r.`role` IN ('3', '4') AND r.`status` = '0' AND r.`id` != $id";
